...
export default function EmployeeProfileClient({
  ...
  const inputStyle = {
    ...
    borderRadius: '7px', fontSize: '13.5px', outline: 'none', boxSizing: 'border-box' as const,
  }
  const labelStyle = {
  ...
  return (
    <div style={{ padding: '32px 40px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '28px' }}>
        {guard.photo_url ? (
          <img src={guard.photo_url} alt={displayName} style={{ width: '64px', height: '64px', borderRadius: '50%', objectFit: 'cover', border: '3px solid #F97316' }} />
        ) : (
          <div style={{ width: '64px', height: '64px', borderRadius: '50%', background: '#FFF7ED', border: '3px solid #F97316', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '24px', fontWeight: 700, color: '#F97316' }}>
        ...
        <div>
          ...
          <p style={{ margin: '2px 0 0', fontSize: '14px', color: '#6B7280' }}>
            {guard.job_title || 'Employee'}{company ? ` · ${company.name}` : ''}
            {guard.location ? ` · ${guard.location}` : ''}
          </p>
        </div>
        <div style={{ marginLeft: 'auto', display: 'flex', gap: '8px' }}>
          <span style={{ padding: '4px 12px', borderRadius: '20px', fontSize: '12px', fontWeight: 600, background: isActive ? '#F0FDF4' : '#F9FAFB', color: isActive ? '#15803D' : '#9CA3AF' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 280px', gap: '20px', alignItems: 'start' }}>
          ...
          {editing && (
            <div style={{ background: '#fff', borderRadius: '12px', border: '1px solid #F97316', boxShadow: '0 1px 3px rgba(249,115,22,0.1)', padding: '24px' }}>
              ...
              <div style={{ display: 'flex', gap: '8px' }}>
                ...
                <button onClick={() => setEditing(false)} style={{ padding: '9px 16px', background: '#F9FAFB', color: '#374151', border: '1px solid #E5E7EB', borderRadius: '8px', fontSize: '13px', cursor: 'pointer' }}>
                  Cancel
                </button>
              </div>
            </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          {/* Employee details */}
          <div style={{ background: '#fff', borderRadius: '12px', border: '1px solid #E5E7EB', boxShadow: '0 1px 3px rgba(0,0,0,0.04)', padding: '20px' }}>
          ...
          {/* QR card */}
          <div style={{ background: '#fff', borderRadius: '12px', border: '1px solid #E5E7EB', boxShadow: '0 1px 3px rgba(0,0,0,0.04)', padding: '20px', textAlign: 'center' }}>
          ...
          {/* Actions */}
          <div style={{ background: '#fff', borderRadius: '12px', border: '1px solid #E5E7EB', boxShadow: '0 1px 3px rgba(0,0,0,0.04)', padding: '20px' }}>
            <p style={{ margin: '0 0 12px', fontSize: '11px', fontWeight: 600, color: '#9CA3AF', textTransform: 'uppercase', letterSpacing: '0.07em' }}>Actions</p>
