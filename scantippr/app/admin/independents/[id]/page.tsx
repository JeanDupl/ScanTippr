import { createClient } from '@supabase/supabase-js'
import { notFound } from 'next/navigation'
import IndependentProfileClient from './IndependentProfileClient'

export const revalidate = 0

export default async function IndependentProfilePage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params
  const supabase = createClient(
    process.env.NEXT_PUBLIC_SUPABASE_URL!,
    process.env.SUPABASE_SERVICE_ROLE_KEY!
  )

  const { data: guard } = await supabase
    .from('guards')
    .select('*')
    .eq('id', id)
    .is('company_id', null)
    .single()

  if (!guard) notFound()

  const { data: transactions } = await supabase
    .from('transactions')
    .select('*')
    .eq('guard_id', id)
    .order('created_at', { ascending: false })

  const completedTx = (transactions ?? []).filter(tx => tx.payment_status === 'complete' || tx.payment_status === 'completed')
  const totalDonations = completedTx.reduce((s, tx) => s + (tx.amount ?? 0), 0)
  const firstOfMonth = new Date(new Date().getFullYear(), new Date().getMonth(), 1).toISOString()
  const thisMonth = completedTx.filter(tx => tx.created_at >= firstOfMonth).reduce((s, tx) => s + (tx.amount ?? 0), 0)
  const fee = Math.min(150, totalDonations)
  const net = totalDonations - fee

  return (
    <IndependentProfileClient
      guard={guard}
      transactions={transactions ?? []}
      totalDonations={totalDonations}
      thisMonth={thisMonth}
      fee={fee}
      net={net}
    />
  )
}
