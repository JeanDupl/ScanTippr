...
function OverflowMenu({ onDeactivate, isActive }: { onDeactivate: () => void, isActive: boolean }) {
  ...
  return (
    <div ref={ref} style={{ position: 'relative' }}>
      <button onClick={() => setOpen(!open)} style={{ padding: '8px 12px', background: '#F3F4F6', color: '#374151', border: 'none', borderRadius: '8px', fontSize: '15px', fontWeight: 700, cursor: 'pointer' }}>
        ···
      </button>
      {open && (
...
export default function IndependentProfileClient({ guard, transactions, totalDonations, thisMonth, fee, net }: {
  guard: any, transactions: any[], totalDonations: number, thisMonth: number, fee: number, net: number
}) {
  ...
  const [editPhone, setEditPhone] = useState(guard.phone ?? '')
  ...
  const saveEdit = async () => {
    ...
    await supabase.from('guards').update({
      ...
      job_title: editJob.trim() || null, phone: editPhone.trim() || null,
      email: editEmail.trim() || null, photo_url,
    }).eq('id', guard.id)
  ...
  const createLogin = async () => {
    if (!guard.email && !editEmail) { setLoginError('No email on file.'); return }
    setLoginStatus('creating'); setLoginError(null)
    try {
      const res = await fetch('/api/create-guard-login', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ guardId: guard.id, email: editEmail || guard.email, companyId: guard.company_id }),
      })
      const data = await res.json()
      if (!res.ok) { setLoginError(data.error || 'Failed'); setLoginStatus('error') }
      else setLoginStatus('success')
    } catch { setLoginError('Could not connect.'); setLoginStatus('error') }
  }
  ...
  return (
    <div style={{ padding: '32px 40px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '20px', marginBottom: '28px', background: '#fff', borderRadius: '12px', border: '1px solid #E5E7EB', padding: '20px 24px', boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
        <div style={{ position: 'relative', flexShrink: 0 }}>
          {photoPreview ? (
            ...
            <div style={{ width: '72px', height: '72px', borderRadius: '50%', background: '#FFF7ED', border: '3px solid #F97316', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '28px', fontWeight: 700, color: '#F97316' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '12px', marginBottom: '24px' }}>
        {[
          { label: 'Total Donations', value: fmtCurrency(totalDonations) },
          { label: 'This Month', value: fmtCurrency(thisMonth) },
          { label: 'ScanTippr Fee', value: fmtCurrency(fee), color: '#B45309' },
          { label: 'Net Amount', value: fmtCurrency(net) },
        ].map(card => (
          <div key={card.label} style={{ background: '#fff', borderRadius: '10px', padding: '16px 20px', border: '1px solid #E5E7EB' }}>
            ...
            <p style={{ margin: 0, fontSize: '20px', fontWeight: 700, color: card.color ?? '#0A0A0A' }}>{card.value}</p>
          </div>
        ))}
      </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          {editing && (
            <div style={{ background: '#fff', borderRadius: '12px', border: '1px solid #F97316', padding: '24px' }}>
              <h3 style={{ margin: '0 0 16px', fontSize: '14px', fontWeight: 600, color: '#0A0A0A' }}>Edit Details</h3>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '12px' }}>
                ...
                <div><label style={labelStyle}>Job Title</label><input value={editJob} onChange={e => setEditJob(e.target.value)} style={inputStyle} /></div>
                <div><label style={labelStyle}>Cell Number</label><input value={editPhone} onChange={e => setEditPhone(e.target.value)} style={inputStyle} /></div>
                <div style={{ gridColumn: 'span 2' }}><label style={labelStyle}>Email</label><input value={editEmail} onChange={e => setEditEmail(e.target.value)} style={inputStyle} /></div>
              </div>
                ...
                {transactions.length === 0 ? (
                  ...
                  return (
                    <tr key={tx.id} style={{ borderBottom: i < transactions.length - 1 ? '1px solid #F9FAFB' : 'none' }}>
                      ...
                      <td style={{ padding: '12px 24px', fontSize: '13.5px', fontWeight: 700, color: '#15803D' }}>{fmtCurrency(tx.amount)}</td>
                      <td style={{ padding: '12px 24px', fontSize: '11px', color: '#9CA3AF', fontFamily: 'monospace' }}>{tx.ozow_payment_id ?? '—'}</td>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          <div style={{ background: '#fff', borderRadius: '12px', border: '1px solid #E5E7EB', padding: '20px', textAlign: 'center' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              ...
              <a href={`/api/qr/${guard.id}?v=4`} target="_blank" style={{ display: 'block', padding: '8px 14px', background: '#F9FAFB', color: '#374151', border: '1px solid #E5E7EB', borderRadius: '7px', fontSize: '13px', textDecoration: 'none' }}>Download QR</a>
            </div>
          </div>
        </div>
      </div>
