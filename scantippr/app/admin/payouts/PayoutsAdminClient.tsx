          ...
          <div style={{ marginTop: '16px', padding: '14px 16px', background: '#FFFBF5', borderRadius: '8px', border: '1px solid #FED7AA', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '16px' }}>
            <label style={{ display: 'flex', alignItems: 'center', gap: '10px', cursor: 'pointer', flex: 1 }}>
              ...
              <span style={{ fontSize: '13px', color: '#374151', lineHeight: 1.4 }}>
                I confirm: run payout for <strong>{selectedCompany?.name}</strong> — <strong>{MONTH_NAMES[periodMonth - 1]} {periodYear}</strong> (<strong>{feeDisposalMode === 'payout_to_scantippr' ? 'fee paid to ScanTippr' : feeDisposalMode === 'remain_in_float' ? 'fee stays in float' : 'fee decision pending'}</strong>). This cannot be undone.
              </span>
            </label>
            <button onClick={handleInitiate} disabled={!canSubmit} style={{ padding: '9px 20px', borderRadius: '8px', border: 'none', background: canSubmit ? '#F97316' : '#E5E7EB', color: canSubmit ? '#fff' : '#9CA3AF', fontSize: '13px', fontWeight: 600, cursor: canSubmit ? 'pointer' : 'not-allowed', whiteSpace: 'nowrap', transition: 'background 0.15s', minWidth: '140px' }}>
        ...
        {state.phase === 'success' && (
          <div style={{ marginTop: '12px', padding: '14px 16px', borderRadius: '8px', background: '#F0FDF4', border: '1px solid #BBF7D0' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '10px' }}>
              ...
              <span style={{ fontSize: '13px', fontWeight: 700, color: '#15803D' }}>Payout initiated — {state.summary.companyName} · {MONTH_NAMES[(state.summary.periodMonth ?? 1) - 1]} {state.summary.periodYear}</span>
            </div>
  ...
  async function handleInitiate() {
    ...
    try {
      const res = await fetch('/api/admin/payouts/initiate-individual', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ guardId, periodMonth, periodYear, feeDisposalMode }),
      })
      const data = await res.json()
      if (!res.ok) {
        setState({ phase: 'error', message: data.error ?? 'Unknown error' })
          ...
          <>
            ...
            {guardId && (
              <div style={{ marginTop: '16px', padding: '14px 16px', background: '#FAFAFF', borderRadius: '8px', border: '1px solid #DDD6FE', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '16px' }}>
                <label style={{ display: 'flex', alignItems: 'center', gap: '10px', cursor: 'pointer', flex: 1 }}>
                  ...
                  <span style={{ fontSize: '13px', color: '#374151', lineHeight: 1.4 }}>
                    I confirm: run payout for <strong>{selectedGuard?.first_name} {selectedGuard?.last_name}</strong> — <strong>{MONTH_NAMES[periodMonth - 1]} {periodYear}</strong> (<strong>{feeDisposalMode === 'payout_to_scantippr' ? 'fee paid to ScanTippr' : feeDisposalMode === 'remain_in_float' ? 'fee stays in float' : 'fee decision pending'}</strong>). This cannot be undone.
                  </span>
                </label>
                <button onClick={handleInitiate} disabled={!canSubmit} style={{ padding: '9px 20px', borderRadius: '8px', border: 'none', background: canSubmit ? '#6D28D9' : '#E5E7EB', color: canSubmit ? '#fff' : '#9CA3AF', fontSize: '13px', fontWeight: 600, cursor: canSubmit ? 'pointer' : 'not-allowed', whiteSpace: 'nowrap', transition: 'background 0.15s', minWidth: '140px' }}>
                  {state.phase === 'loading' ? 'Initiating…' : 'Initiate Payout'}
                </button>
              </div>
            )}
            ...
            {state.phase === 'success' && (
              <div style={{ marginTop: '12px', padding: '14px 16px', borderRadius: '8px', background: '#F0FDF4', border: '1px solid #BBF7D0' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '10px' }}>
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#15803D" strokeWidth="2.5"><polyline points="20 6 9 17 4 12"/></svg>
                  <span style={{ fontSize: '13px', fontWeight: 700, color: '#15803D' }}>Payout initiated — {state.summary.guardName} · {MONTH_NAMES[(state.summary.periodMonth ?? 1) - 1]} {state.summary.periodYear}</span>
                </div>
                <div style={{ display: 'flex', gap: '24px' }}>
...
export default function PayoutsAdminClient({
  ...
  const filtered = useMemo(() => periods.filter(p => {
    ...
    return inRecipient && inType && inStatus && inMonth && inYear && inSearch
  }), [periods, recipientFilter, typeFilter, statusFilter, monthFilter, yearFilter, search])
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ background: '#FAFAFA' }}>
              {['Period', 'Recipient', 'Type', 'Employees', 'Gross', 'Fee', 'Net Payout', 'Status', ''].map(h => (
